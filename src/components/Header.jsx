...
const Header = ({ account }) => {
 ...
 function shortenAddress(address) {
    ...
    const showInfo = () => {
        toast.info('Disconnect account from wallet to log out !', {
            position: 'top-center',
            ...
            <nav className="bg-white border-gray-200 px-4 lg:px-6 py-2.5 dark:bg-gray-800">
                <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
                    <Link to="/" className="flex items-center">
                        <img src={icon} className="mr-3 h-6 sm:h-9" alt="Ether Wallet Logo" />
                        <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-white">EtherWallet</span>
                    </Link>
                    <div className="flex items-center lg:order-2">
                        <span className="text-gray-800 dark:text-white font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2">Address : </span>
                        <h2 className="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">{shortenAddress(account)}</h2>
                        <button onClick={showMsg} className="text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-4 lg:px-5 py-2 lg:py-2.5 mr-2 dark:bg-red-600 dark:hover:bg-red-700 focus:outline-none dark:focus:ring-red-800">Log Out</button>
